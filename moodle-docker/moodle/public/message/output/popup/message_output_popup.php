 ...
 *
 * @package   message_popup
 * @copyright 2008 Luis Rodrigues and Ayoub Ben Chahla
...
class message_output_popup extends message_output
{
    ...
    public function cleanup_all_notifications(int $notificationdeletetime): void
    {
        ...
        $DB->delete_records_select(
            ...
            'notificationid IN (SELECT id FROM {notifications} WHERE timecreated < ?)',
    ...
    public function cleanup_read_notifications(int $notificationdeletetime): void
    {
        ...
        $DB->delete_records_select(
            'message_popup_notifications',
            'notificationid IN (SELECT id FROM {notifications} WHERE timeread < ?)',
            [$notificationdeletetime]
        );
    }
}
