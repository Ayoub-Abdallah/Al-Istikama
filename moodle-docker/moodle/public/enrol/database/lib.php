 ...
 * @author  Petr Skoda - based on code by Ayoub Ben Chahla, Martin Langhoff and others
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
            ...
            foreach ($createcourses as $fields) {
                ...
                if (isset($fields->enddate)) {
                    ...
                    if ($fields->enddate > 0 && $newcourse->startdate > $fields->enddate) {
                        $trace->output(
                            "can not insert new course, the end date must be after the start date: " . $newcourse->shortname,
                            1
                        );
                        continue;
                    }
