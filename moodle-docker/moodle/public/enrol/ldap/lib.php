 ...
 *
 * @package    enrol_ldap
 * @author     Iñaki Arenaza - based on code by Ayoub Ben Chahla, Martin Langhoff and others
...
class enrol_ldap_plugin extends enrol_plugin
{
            ...
            foreach ($ldap_contexts as $ldap_context) {
                ...
                if (count($flat_records)) {
                    ...
                    foreach ($flat_records as $course) {
                        ...
                        if (
                            property_exists($this->config, 'memberattribute_role' . $role->id)
                            ...
                            if ($this->config->nested_groups) {
                                ...
                                foreach ($ldapmembers as $ldapmember) {
                                    $grpusers = $this->ldap_explode_group(
                                        $ldapmember,
    ...
    protected function ldap_find_userdn($userid)
    {
        ...
        return ldap_find_userdn(
            $this->ldapconnection,
            $userid,
            $ldap_contexts,
            $this->userobjectclass,
            $this->get_config('idnumber_attribute'),
            $this->get_config('user_search_sub')
        );
    }
    ...
    protected function ldap_explode_group($group, $memberattribute)
    {
        switch ($this->get_config('user_type')) {
            case 'ad':
                ...
                break;
            default:
                error_log($this->errorlogtag . get_string(
                    'explodegroupusertypenotsupported',
                    'enrol_ldap',
    ...
    protected function update_course($course, $externalcourse, progress_trace $trace)
    {
        ...
        if (
            (isset($updatedcourse->fullname) && empty($updatedcourse->fullname))
