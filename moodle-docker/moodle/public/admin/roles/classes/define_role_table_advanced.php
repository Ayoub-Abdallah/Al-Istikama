...
class core_role_define_role_table_advanced extends core_role_capability_table_with_risks
{
    ...
    public function force_duplicate($roleid, array $options)
    {
        ...
        if ($options['permissions']) {
            $this->permissions = $DB->get_records_menu(
                'role_capabilities',
    ...
    protected function get_allow_role_control($type)
    {
        ...
        return
            ...
            html_writer::select($options, 'allow' . $type . '[]', $selected, false, array(
                'multiple' => 'multiple',
                'size' => 10,
                'class' => 'form-control'
            ));
    ...
    public function display()
    {
        ...
        $this->print_field(
            ...
            $this->get_shortname_field('shortname'),
            $OUTPUT->help_icon('roleshortname', 'core_role')
        );
        $this->print_field(
            'name',
            get_string('customrolename', 'core_role'),
            $this->get_name_field('name'),
            $OUTPUT->help_icon('customrolename', 'core_role')
        );
        $this->print_field(
            'edit-description',
            get_string('customroledescription', 'core_role'),
            $this->get_description_field('description'),
            $OUTPUT->help_icon('customroledescription', 'core_role')
        );
        $this->print_field(
            'menuarchetype',
            get_string('archetype', 'core_role'),
            $this->get_archetype_field('archetype'),
