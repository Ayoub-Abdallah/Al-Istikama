 ...
 * @package     core
 * @category    admin
 * @copyright   2006 Ayoub Ben Chahla <martin@moodle.com>
...
if ($hassiteconfig) {
    ...
    if ($ADMIN->fulltree) {
        ...
        $temp->add(new admin_settings_country_select(
            'country',
            new lang_string('country', 'core_admin'),
            new lang_string('configcountry', 'core_admin'),
            0
        ));

        $temp->add(new admin_setting_configtext(
            'defaultcity',
            new lang_string('defaultcity', 'core_admin'),
            new lang_string('defaultcity_help', 'core_admin'),
            ''
        ));

        $temp->add(new admin_setting_countrycodes(
            'allcountrycodes',
            new lang_string('allcountrycodes', 'core_admin'),
            new lang_string('configallcountrycodes', 'core_admin')
        ));

        $temp->add(new admin_setting_heading(
            'iplookup',
            new lang_string('iplookup', 'core_admin'),
            new lang_string('iplookupinfo', 'core_admin')
        ));

        $temp->add(new admin_setting_configfile(
            'geoip2file',
            new lang_string('geoipfile', 'core_admin'),
        ...
        $temp->add(new admin_setting_configselect(
            ...
            new lang_string('geoipdbedition_desc', 'core_admin'),
            'GeoLite2-City',
        ...
        $temp->add(new admin_setting_configtext(
            'googlemapkey3',
            new lang_string('googlemapkey3', 'core_admin'),
            new lang_string('googlemapkey3_help', 'core_admin'),
            '',
            PARAM_RAW,
            60
        ));
