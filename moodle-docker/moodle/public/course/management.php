...
// Istikama: redirect target users to the custom school management page.
if (file_exists($CFG->dirroot . '/local/istikama_admin/locallib.php')) {
    require_once($CFG->dirroot . '/local/istikama_admin/locallib.php');
    if (isloggedin() && !isguestuser() && local_istikama_admin_is_target_user()) {
        redirect(new moodle_url('/local/istikama_admin/schools.php'));
    }
}
