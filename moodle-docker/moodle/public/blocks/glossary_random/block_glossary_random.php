...
class block_glossary_random extends block_base
{
    ...
    function specialization()
    {
        ...
        if (time() > $this->config->nexttime) {
            ...
            if (
                !$numberofentries = $DB->count_records(
                    'glossary_entries',
            ...
            if (empty($entries)) {
                ...
                if (isset($entry->definitiontrust) && isset($entry->id) && isset($entry->definition)) {
                    $entry->definition = file_rewrite_pluginfile_urls(
                        $entry->definition,
                        'pluginfile.php',
                        $glossaryctx->id,
                        'mod_glossary',
        ...
        if ($cm->uservisible) {
            ...
            if (has_capability('mod/glossary:write', context_module::instance($cm->id))) {
                $this->content->footer = html_writer::link(
                    new moodle_url('/mod/glossary/edit.php', ['cmid' => $cm->id]),
                    format_string($this->config->addentry)
                ) . '<br/>';
            }
