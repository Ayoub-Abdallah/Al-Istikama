...
class block_online_users extends block_base
{
    ...
    function get_content()
    {
        ...
        if (
            !has_capability('block/online_users:viewlist', $this->page->context)
        ...
        if (!empty($users)) {
            ...
            if (
                isloggedin() && has_capability('moodle/site:sendmessage', $this->page->context)
                ...
                if (isguestuser($user)) {
                    ...
                    if ($USER->id == $user->id) {
                        if ($CFG->block_online_users_onlinestatushiding) {
                            ...
                            $anchortagcontents = $OUTPUT->pix_icon(
                                't/' . $action,
                                get_string('online_status:' . $action, 'block_online_users')
                            );
                            $anchortag = html_writer::link(
                                "",
                                $anchortagcontents,
                                array(
                                    'title' => get_string('online_status:' . $action, 'block_online_users'),
                                    'data-action' => $action,
                                    'data-userid' => $user->id,
                                    'id' => 'change-user-visibility'
                                )
                            );
