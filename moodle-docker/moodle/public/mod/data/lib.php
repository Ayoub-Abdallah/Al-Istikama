...
{     // Base class for Database Field Types (see field/*/field.class.php)
    ...
    {   // Field or data or both, each can be id or object
        global $DB;
...
function data_generate_tag_form($recordid = false, $selected = [])
{
    ...
    if ($showstandard) {
        $tags += $DB->get_records_menu(
            'tag',
            array('isstandard' => 1, 'tagcollid' => $tagcollid),
            $namefield,
            'id,' . $namefield . ' as fieldname'
        );
    }
    ...
    if (has_capability('moodle/tag:manage', context_system::instance()) && $showstandard) {
        $url = new moodle_url('/tag/manage.php', array(
            'tc' => core_tag_area::get_collection(
                'mod_data',
                'data_records'
            )
        ));
...
function data_print_preference_form($data, $perpage, $search, $sort = '', $order = 'ASC', $search_array = '', $advanced = 0, $mode = '')
{
    ...
    echo html_writer::select($pagesizes, 'perpage', $perpage, false, array(
        'id' => 'pref_perpage',
...
function data_convert_to_roles($data, $teacherroles = array(), $studentroles = array(), $cmid = NULL)
{
    ...
    if (
        !isset($data->participants) && !isset($data->assesspublic)
...
function data_reset_userdata($data)
{
    ...
    if (!empty($data->reset_data_notenrolled)) {
        ...
        foreach ($rs as $record) {
            if (
                array_key_exists($record->userid, $notenrolled) or !$record->userexists or $record->userdeleted
...
function data_extend_settings_navigation(settings_navigation $settings, navigation_node $datanode)
{
    ...
    if (has_capability(DATA_CAP_EXPORT, $settings->get_page()->cm->context)) {
        ...
        $exportentriesnode = $datanode->add(
    ...
    if (has_capability('mod/data:manageentries', $settings->get_page()->cm->context)) {
        $importentriesnode = $datanode->add(
 ...
 */
function mod_data_core_calendar_provide_event_action(
