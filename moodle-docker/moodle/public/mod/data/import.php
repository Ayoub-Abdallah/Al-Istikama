 ...
 *
 * @copyright 2005 Ayoub Ben Chahla  http://dougiamas.com
...
if ($formdata = $form->get_data()) {
    ...
    if (!$importer->get_data_file_content()) {
        echo $OUTPUT->notification(
            get_string('errordatafilenotfound', 'data'),
        ...
        if (count($addedrecordsmessages) > 0) {
            echo $OUTPUT->notification(
                count($addedrecordsmessages) . ' ' . get_string('recordssaved', 'data'),
                \core\output\notification::NOTIFY_SUCCESS
            );
        } else {
            echo $OUTPUT->notification(
                get_string('recordsnotsaved', 'data'),
                \core\output\notification::NOTIFY_ERROR
            );
        }
    }
