...
function glossary_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames)
{
    ...
    if (!empty($activity->user)) {
        echo html_writer::tag(
            'div',
            $OUTPUT->user_picture($activity->user, array('courseid' => $courseid)),
    ...
    echo html_writer::link(
        new moodle_url('/mod/glossary/view.php', $urlparams),
        strip_tags($activity->content->concept),
        $class
    );
...
function glossary_print_entry_icons($course, $cm, $glossary, $entry, $mode = '', $hook = '', $type = 'print')
{
    ...
    if (!$entry->approved) {
        ...
        $return .= html_writer::tag(
            'span',
            get_string('entryishidden', 'glossary'),
 ...
 * @return void
 */
function glossary_print_entry_lower_section(
    $course,
    $cm,
    $glossary,
    $entry,
...
function glossary_reset_course_form_definition(&$mform)
{
    ...
    $mform->addElement(
        ...
        get_string('resetglossaries', 'glossary'),
        array('main' => get_string('mainglossary', 'glossary'), 'secondary' => get_string('secondaryglossary', 'glossary')),
...
function glossary_reset_userdata($data)
{
    ...
    if (!empty($data->reset_glossary_notenrolled)) {
        ...
        if ($rs->valid()) {
            foreach ($rs as $entry) {
                if (
                    array_key_exists($entry->userid, $notenrolled) || !$entry->userexists || $entry->userdeleted
    ...
    foreach ($showtabs as $showtabkey => $showtabvalue) {
        ...
        switch ($showtabvalue) {
            case GLOSSARY_STANDARD:
                $navigation->add(get_string('standardview', 'glossary'), new moodle_url(
                    '/mod/glossary/view.php',
                    array('id' => $cm->id, 'mode' => 'letter')
                ));
                break;
            case GLOSSARY_CATEGORY:
                $navigation->add(get_string('categoryview', 'glossary'), new moodle_url(
                    '/mod/glossary/view.php',
                    array('id' => $cm->id, 'mode' => 'cat')
                ));
                break;
            case GLOSSARY_DATE:
                $navigation->add(get_string('dateview', 'glossary'), new moodle_url(
                    '/mod/glossary/view.php',
                    array('id' => $cm->id, 'mode' => 'date')
                ));
                break;
            case GLOSSARY_AUTHOR:
                $navigation->add(get_string('authorview', 'glossary'), new moodle_url(
                    '/mod/glossary/view.php',
...
function glossary_extend_settings_navigation(settings_navigation $settings, navigation_node $glossarynode)
{
    ...
    if (has_capability('mod/glossary:export', $settings->get_page()->cm->context)) {
        $node = $glossarynode->add(
            ...
            new moodle_url('/mod/glossary/export.php', [
                'id' => $settings->get_page()->cm->id,
                'mode' => $mode,
                'hook' => $hook
            ])
        );
    ...
    $hiddenentries = $DB->count_records('glossary_entries', [
        'glossaryid' => $settings->get_page()->cm->instance,
        'approved' => 0
    ]);
    ...
    if (
        ...
        $glossarynode->add(
            get_string('pendingapprovalcount', 'glossary', $hiddenentries),
    ...
    if (
        ...
        $url = new moodle_url(rss_get_url(
            $settings->get_page()->cm->context->id,
            $USER->id,
            'mod_glossary',
            $glossary->id
        ));
 ...
 */
function mod_glossary_core_calendar_provide_event_action(
 ...
 * @throws moodle_exception
 */
function mod_glossary_can_update_entry(
    stdClass $entry,
    stdClass $glossary,
    stdClass $context,
    object $cm,
