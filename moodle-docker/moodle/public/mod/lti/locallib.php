...
// for Moodle. Moodle is a Free Open source Learning Management System by Ayoub Ben Chahla.
...
function lti_build_request(
    ...
    if (
        !empty($instance->id) && !empty($instance->servicesalt) && ($islti2 ||
    ...
    if (
        $islti2 || $typeconfig['sendname'] == LTI_SETTING_ALWAYS ||
    ...
    if (
        $islti2 || $typeconfig['sendemailaddr'] == LTI_SETTING_ALWAYS ||
...
function lti_build_custom_parameters($toolproxy, $tool, $instance, $params, $customstr, $instructorcustomstr, $islti2)
{
    ...
    if ($instructorcustomstr) {
        $custom = array_merge(lti_split_custom_parameters(
            $toolproxy,
            $tool,
            $params,
            $instructorcustomstr,
    ...
    if (!empty($tools)) {
        ...
        foreach ($tools as $type) {
            ...
            $accepthtml = $OUTPUT->action_icon(
                $baseurl,
                new \pix_icon('t/check', $accept, '', array('class' => 'iconsmall')),
                null,
            ...
            $updatehtml = $OUTPUT->action_icon(
                $updateurl,
                new \pix_icon('t/edit', $update, '', array('class' => 'iconsmall')),
                null,
            ...
            if (($type->state != LTI_TOOL_STATE_REJECTED) || empty($type->toolproxyid)) {
                ...
                $deletehtml = $OUTPUT->action_icon(
                    $deleteurl,
                    new \pix_icon('t/delete', $delete, '', array('class' => 'iconsmall')),
                    null,
...
{
    debugging(
        __FUNCTION__ . '() is deprecated. Please use \mod_lti\local\types_helper::get_lti_types_by_course() instead.',
        DEBUG_DEVELOPER
    );
...
function lti_get_configured_types($courseid, $sectionreturn = 0)
{
    global $OUTPUT, $USER;
    $types = [];
    $preconfiguredtypes = \mod_lti\local\types_helper::get_lti_types_by_course(
        $courseid,
        $USER->id,
...
function lti_get_shared_secrets_by_key($key)
{
    ...
    $sharedsecrets = $DB->get_records_sql($query, array(
        'configured1' => LTI_TOOL_STATE_CONFIGURED,
        'ltiversion' => $lti13,
        'configured2' => LTI_TOOL_STATE_CONFIGURED,
        'key1' => $key,
        'key2' => $key,
        'key3' => $key
    ));
...
function lti_get_tool_settings($toolproxyid, $courseid = null, $instanceid = null)
{
    ...
    $settings = array();
    if ($toolproxyid > 0) {
        $settingsstr = $DB->get_field('lti_tool_settings', 'settings', array(
...
function lti_set_tool_settings($settings, $toolproxyid, $courseid = null, $instanceid = null)
{
    ...
    if ($toolproxyid >= 0) {
        $record = $DB->get_record('lti_tool_settings', array(
            'toolproxyid' => $toolproxyid,
            'course' => $courseid,
            'coursemoduleid' => $instanceid
        ));
