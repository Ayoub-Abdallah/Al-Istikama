...
$url = new moodle_url('/mod/scorm/report/userreporttracks.php', array(
    'id' => $id,
    'user' => $userid,
    'attempt' => $attempt,
    'scoid' => $scoid,
...
$PAGE->navbar->add(
    "$strattempt $attempt - " . fullname($user),
...
foreach ($trackdata as $element => $value) {
    if (substr($element, 0, 3) == 'cmi') {
        ...
        if (stristr($element, '.id') !== false) {
            ...
            stristr($element, '.student_response') !== false or // SCORM 1.2 value.
            stristr($element, '.learner_response') !== false
        ) { // SCORM 2004 value.
