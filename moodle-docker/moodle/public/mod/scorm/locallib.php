...
function scorm_get_popup_options_array()
{
    ...
    return array(
        'scrollbars' => isset($cfgscorm->scrollbars) ? $cfgscorm->scrollbars : 0,
...
{
    return array(
        SCORM_SKIPVIEW_NEVER => get_string('never'),
...
{
    return array(
        SCORM_TOC_SIDE => get_string('sided', 'scorm'),
...
{
    return array(
        SCORM_UPDATE_NEVER => get_string('never'),
...
{
    return array(
        0 => get_string('currentwindow', 'scorm'),
...
{
    return array(
        SCORM_NAV_DISABLED => get_string('no'),
...
{
    return array(
        SCORM_DISPLAY_ATTEMPTSTATUS_NO => get_string('no'),
...
{
    return array(
        SCORM_FORCEATTEMPT_NO => get_string('no'),
...
function scorm_insert_track($userid, $scormid, $scoid, $attemptornumber, $element, $value, $forcecompleted = false, $trackdata = null)
{
    ...
    $scorm = false;
    if (
        in_array($element, ['cmi.core.score.raw', 'cmi.score.raw']) ||
    ...
    if (
        ...
        $data = [
            'other' => ['attemptid' => $attempt->id, 'cmielement' => $element, 'cmivalue' => $value],
            'objectid' => $scorm->id,
...
function scorm_get_attempt_status($user, $scorm, $cm = '')
{
    ...
    if (!empty($cm)) {
        $context = context_module::instance($cm->id);
        if (
            has_capability('mod/scorm:deleteownresponses', $context) &&
                    ...
                    if (isset($usertracks[$sco->identifier])) {
                        ...
                        if ($sco->scormtype == 'sco') {
                            ...
                            $statusicon = $OUTPUT->pix_icon(
                                $statusstringexists ? $usertrack->status : 'incomplete',
                                $strstatus,
                                'scorm'
                            );
                        } else {
...
function scorm_format_toc_for_treeview(
    $user,
    $scorm,
    $scoes,
    $usertracks,
                ...
                if (!empty($prevsco) && scorm_version_check($scorm->version, SCORM_13) && !empty($prevsco->hidecontinue)) {
                    ...
                    if (!empty($sco->launch)) {
                        if ($sco->scormtype == 'sco') {
                            ...
                            $result->toc .= '&nbsp;' . html_writer::link(
                                $url,
                                format_string($sco->title),
                                array('data-scoid' => $sco->id)
                            ) . $score;
                        }
                    } else {
                        if ($sco->scormtype == 'sco') {
...
function scorm_format_toc_for_droplist(
    $scorm,
    $scoes,
    $usertracks,
    $currentorg = '',
    ...
    if (!empty($scoes)) {
        ...
        foreach ($scoes as $sco) {
            ...
            if (!empty($sco->children)) {
                $tocmenus = scorm_format_toc_for_droplist(
                    $scorm,
                    $sco->children,
                    $usertracks,
                    $currentorg,
...
function scorm_get_toc(
    ...
    $treeview = scorm_format_toc_for_treeview(
        $user,
        $scorm,
    ...
    if ($scorm->hidetoc == SCORM_TOC_POPUP) {
        $tocmenu = scorm_format_toc_for_droplist(
            $scorm,
            $scoes['scoes'][0]->children,
            $scoes['usertracks'],
            $currentorg,
