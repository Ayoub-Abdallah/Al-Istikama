 ...
 */
function quiz_start_new_attempt(
    $quizobj,
    $quba,
    $attempt,
    $attemptnumber,
...
function quiz_repaginate_questions($quizid, $slotsperpage)
{
    ...
    foreach ($slots as $slot) {
        if (
            ($firstslots && in_array($slot->slot, $firstslots)) ||
...
function quiz_question_preview_url($quiz, $question, $variant = null, $restartversion = null)
{
    ...
    $displayoptions = display_options::make_from_quiz(
        $quiz,
...
function quiz_get_review_options($quiz, $attempt, $context)
{
    ...
    if (
        !empty($attempt->id) && $attempt->state == quiz_attempt::FINISHED && !$attempt->preview &&
...
function quiz_get_combined_reviewoptions($quiz, $attempts)
{
    ...
    foreach ($attempts as $attempt) {
        $attemptoptions = display_options::make_from_quiz(
            $quiz,
...
function quiz_attempt_submitted_handler($event)
{
    ...
    return quiz_send_notification_messages(
        $course,
        $quiz,
        $attempt,
        context_module::instance($cm->id),
        $cm,
        $eventdata['other']['studentisonline']
    );
}
 ...
 */
function quiz_question_tostring(
    $question,
    $showicon = false,
...
function quiz_add_quiz_question($questionid, $quiz, $page = 0, $maxmark = null)
{
    ...
    $questionslots = $DB->get_records_sql($sql, [
        $quiz->id,
        'mod_quiz',
        'slot',
...
function quiz_update_section_firstslots($quizid, $direction, $afterslot, $beforeslot = null)
{
    ...
    $firstslotschanges = $DB->get_records_select_menu(
        'quiz_sections',
        $where,
        $params,
        '',
 ...
 * @since  Moodle 3.1
 */
function quiz_prepare_and_start_new_attempt(
    quiz_settings $quizobj,
    $attemptnumber,
    $lastattempt,
