...
function quiz_grade_item_update($quiz, $grades = null)
{
    ...
    $openreviewoptions = display_options::make_from_quiz(
        $quiz,
        display_options::LATER_WHILE_OPEN
    );
    $closedreviewoptions = display_options::make_from_quiz(
        $quiz,
    ...
    if (isset($quiz->feedbacktext)) {
        ...
        if (!empty($quiz->feedbackboundaries)) {
            for ($i = $numboundaries; $i < count($quiz->feedbackboundaries); $i += 1) {
                if (
                    !empty($quiz->feedbackboundaries[$i]) &&
        ...
        for ($i = $numboundaries + 1; $i < count($quiz->feedbacktext); $i += 1) {
            if (
                !empty($quiz->feedbacktext[$i]['text']) &&
...
function quiz_reset_userdata($data)
{
    ...
    if (!empty($data->reset_quiz_attempts)) {
        question_engine::delete_questions_usage_by_activities(new qubaid_join(
            ...
            'quiza.uniqueid',
            'quiz.course = :quizcourseid',
            ['quizcourseid' => $data->courseid]
        ));

        $DB->delete_records_select(
            'quiz_attempts',
            'quiz IN (SELECT id FROM {quiz} WHERE course = ?)',
            [$data->courseid]
        );
        $status[] = [
    ...
    if (!empty($data->reset_quiz_user_overrides)) {
        $DB->delete_records_select(
            'quiz_overrides',
            'quiz IN (SELECT id FROM {quiz} WHERE course = ?) AND userid IS NOT NULL',
            [$data->courseid]
        );
        $status[] = [
            'component' => $componentstr,
    ...
    if (!empty($data->reset_quiz_group_overrides)) {
        $DB->delete_records_select(
            'quiz_overrides',
            'quiz IN (SELECT id FROM {quiz} WHERE course = ?) AND groupid IS NOT NULL',
            [$data->courseid]
        );
        $status[] = [
            'component' => $componentstr,
...
function quiz_extend_settings_navigation(settings_navigation $settings, navigation_node $quiznode)
{
    ...
    if (has_any_capability(['mod/quiz:viewreports', 'mod/quiz:grade'], $settings->get_page()->cm->context)) {
        ...
        $reportnode = $quiznode->add_node(navigation_node::create(
 ...
 * @return bool false if file not found, does not return if found - justsend the file
 */
function quiz_question_pluginfile(
    $course,
    $context,
    $component,
    $filearea,
 ...
 */
function mod_quiz_core_calendar_provide_event_action(
