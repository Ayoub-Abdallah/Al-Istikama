    ...
    public function load_data()
    {
        ...
        if ($this->post) {
            if ($this->attachment) {
                ...
                if (
                    $file->get_contextid() != $this->modcontext->id
    ...
    function prepare_package()
    {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry(
                    'forumdiscussion' . $this->discussionid,
                    get_string('discussion', 'forum') . ': ' . $this->discussion->name,
                    'selection'
                );
...
class forum_file_info_container extends file_info
{
    ...
    private function get_filtered_children($extensions = '*', $countonly = false, $returnemptyfolders = false)
    {
        ...
        foreach ($rs as $record) {
            if (
                ($child = $this->browser->get_file_info($this->context, 'mod_forum', $this->filearea, $record->itemid))
