...
if ($issubscribed) {
    if (is_null($sesskey)) {
        ...
        if ($discussionid) {
            ...
            echo $OUTPUT->confirm(
                get_string('confirmunsubscribediscussion', 'forum', $a),
                $PAGE->url,
                $viewurl
            );
        } else {
            echo $OUTPUT->confirm(
                get_string('confirmunsubscribe', 'forum', format_string($forum->name)),
                $PAGE->url,
                $viewurl
            );
        }
    ...
    if (is_null($sesskey)) {
        ...
        if ($discussionid) {
            ...
            $a->discussion = format_string($discussion->name);
            echo $OUTPUT->confirm(
                get_string('confirmsubscribediscussion', 'forum', $a),
                $PAGE->url,
                $viewurl
            );
        } else {
            echo $OUTPUT->confirm(
                get_string('confirmsubscribe', 'forum', format_string($forum->name)),
                $PAGE->url,
                $viewurl
            );
        }
