...
function forum_print_recent_activity($course, $viewfullnames, $timestart)
{
    ...
    if (
        !$posts = $DB->get_records_sql("SELECT p.*,
...
function forum_get_all_discussion_posts($discussionid, $sort, $tracking = false)
{
    ...
    $params[] = $discussionid;
    if (
        !$posts = $DB->get_records_sql("SELECT p.*, $allnames, u.email, u.picture, u.imagealt $tr_sel
                                     FROM {forum_posts} p
                                          LEFT JOIN {user} u ON p.userid = u.id
                                          $tr_join
                                    WHERE p.discussion = ?
...
function forum_search_posts(
    ...
    if (in_array('starredonly:on', $searchterms)) {
        ...
        list($favjoin, $favparams) = $ufservice->get_join_sql_by_type(
            'mod_forum',
            'discussions',
            "favourited",
            "d.id"
    ...
    if ($lexer->parse($searchstring)) {
        ...
        if ($parsearray) {
            ...
            list($messagesearch, $msparams) = search_generate_SQL(
                $parsearray,
                'p.message',
                'p.subject',
                'p.userid',
 ...
 * @return  array
 */
function forum_count_discussion_replies(
    $forumid,
    $forumsort = "",
    $limit = -1,
    $page = -1,
 ...
 * @return array
 */
function forum_get_discussions(
    $cm,
    $forumsort = "",
    $fullpost = true,
    $unused = -1,
...
function forum_move_attachments($discussion, $forumfrom, $forumto)
{
    ...
    if ($posts = $DB->get_records('forum_posts', array('discussion' => $discussion->id), '', 'id, attachment')) {
        foreach ($posts as $post) {
            ...
            $attachmentsmoved = $fs->move_area_files_to_new_context(
                $oldcontext->id,
                $newcontext->id,
                'mod_forum',
                'attachment',
    ...
    if ($files) {
        ...
        foreach ($files as $file) {
            ...
            if (!empty($CFG->enableplagiarism)) {
                ...
                $output .= plagiarism_get_links(array(
                    'userid' => $post->userid,
                    'file' => $file,
                    'cmid' => $cm->id,
                    'course' => $cm->course,
...
function forum_update_post($newpost, $mform, $unused = null)
{
    ...
    $post->message = file_save_draft_area_files(
        ...
        'post',
        $post->id,
        mod_forum_post_form::editor_options($context, $post->id),
        $post->message
    );
    \mod_forum\local\entities\post::add_message_counts($post);
    $DB->update_record('forum_posts', $post);
 ...
 * @return string
 */
function forum_get_subscribe_link(
    $forum,
    $context,
    $messages = array(),
...
function forum_user_can_see_post($forum, $discussion, $post, $user = null, $cm = null, $checkdeleted = true)
{
    ...
    if ($forum->type == 'qanda') {
        if (
            has_capability('mod/forum:viewqandawithoutposting', $modcontext, $user->id) || $post->userid == $user->id
...
function forum_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid = 0, $groupid = 0)
{
    ...
    foreach ($posts as $post) {
        ...
        if (
            !empty($CFG->forum_enabletimedposts) and $USER->id != $post->duserid
...
{
    return array(
        'news' => get_string('namenews', 'forum'),
...
function forum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode)
{
    ...
    if ($enrolled && forum_tp_can_track_forums($forumobject)) { // keep tracking info for users with suspended enrolments
        if (
            $forumobject->trackingtype == FORUM_TRACKING_OPTIONAL
    ...
    if ($enablerssfeeds && $forumobject->rsstype && $forumobject->rssarticles && $hascourseaccess) {
        ...
        $url = new moodle_url(rss_get_url(
            $settingsnav->get_page()->cm->context->id,
            $userid,
            "mod_forum",
            $forumobject->id
        ));
...
function forum_get_posts_by_user($user, array $courses, $musthaveaccess = false, $discussionsonly = false, $limitfrom = 0, $limitnum = 50)
{
    ...
    foreach ($courses as $course) {
        ...
        if ($iscurrentuser || $hascapsonuser) {
            ...
            if (
                groups_get_course_groupmode($course) == SEPARATEGROUPS && $course->groupmodeforce
...
function forum_get_context($forumid, $context = null)
{
    ...
    if (!$context || !($context instanceof context_module)) {
        ...
        if (
            $PAGE->cm && $PAGE->cm->modname === 'forum' && $PAGE->cm->instance == $forumid
...
function forum_can_create_attachment($forum, $context)
{
    ...
    if (
        empty($forum->maxattachments) || $forum->maxbytes == 1 ||
 ...
 */
function mod_forum_core_calendar_provide_event_action(
 ...
 */
function mod_forum_get_discussion_summaries(
    \mod_forum\local\entities\forum $forum,
    stdClass $user,
    ?int $groupid,
    ?int $sortorder,
    ?int $pageno = 0,
