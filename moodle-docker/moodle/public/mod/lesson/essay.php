...
switch ($mode) {
    ...
    case 'update':
        ...
        $essayinfo = file_prepare_standard_editor(
            $essayinfo,
            'response',
            $editoroptions,
            $context,
    ...
    case 'email':
        ...
        foreach ($attempts as $attempt) {
            ...
            if ($essayinfo->graded && !$essayinfo->sent) {
                ...
                $answer = file_rewrite_pluginfile_urls(
                    $essayinfo->answer,
                    'pluginfile.php',
                    $context->id,
                    'mod_lesson',
...
switch ($mode) {
    case 'display':
        ...
        foreach (array_keys($studentessays) as $userid) {
            ...
            $table->data[] = array(
                $OUTPUT->user_picture($users[$userid], array('courseid' => $course->id)) . $studentname,
                implode("<br />", $essaylinks),
                implode("<br />", $essaystatuses),
                $emaillink
            );
        }
    ...
    case 'grade':
        ...
        $data->studentanswer = format_text(
            $answer,
            $essayinfo->answerformat,
            array('context' => $context, 'para' => true)
        );
