...
function lesson_grade($lesson, $ntries, $userid = 0)
{
    ...
    if (
        $useranswers = $DB->get_records_select("lesson_attempts", "lessonid = :lessonid AND
...
function lesson_get_overview_report_table_and_data(lesson $lesson, $currentgroup)
{
    ...
    foreach ($attempts as $attempt) {
        ...
        if (empty($studentdata[$attempt->userid]) || empty($studentdata[$attempt->userid][$attempt->retry])) {
            ...
            $studentdata[$attempt->userid][$attempt->retry] = array(
                "timestart" => $timestart,
                "timeend" => $timeend,
                "grade" => $usergrade,
                "end" => $eol,
    ...
    foreach ($branches as $branch) {
        ...
        if (empty($studentdata[$branch->userid]) || empty($studentdata[$branch->userid][$branch->retry])) {
            ...
            // Build up the array.
            // This array represents each student and all of their tries at the lesson.
            $studentdata[$branch->userid][$branch->retry] = array(
                "timestart" => $timestart,
                "timeend" => $timeend,
                "grade" => $usergrade,
                "end" => $eol,
...
class lesson extends lesson_base
{
    ...
    public function cluster_jump($pageid, $userid = null)
    {
        ...
        foreach ($clusterpages as $key => $cluster) {
            ...
            if (
                $this->is_sub_page_of_type(
                    $cluster->id,
                    array(LESSON_PAGE_BRANCHTABLE),
                    array(LESSON_PAGE_ENDOFBRANCH, LESSON_PAGE_CLUSTER)
                )
                    ...
                    if (isset($USER->modattempts[$this->properties->id])) { // If reviewing, make sure update old grade record.
                        if (
                            !$grades = $DB->get_records(
                                "lesson_grades",
                                array("lessonid" => $this->properties->id, "userid" => $USER->id),
                                "completed DESC",
                                '*',
                                0,
                                1
...
abstract class lesson_page extends lesson_base
{
    ...
    final public function record_attempt($context)
    {
        ...
        if ($result->noanswer) {
            ...
            if (!has_capability('mod/lesson:manage', $context)) {
                ...
                $nattempts = $DB->count_records('lesson_attempts', array(
            ...
            if ($result->response) {
                ...
                $result->feedback .= $OUTPUT->box(
                    format_text($this->get_contents(), $this->properties->contentsformat, $options),
    ...
    private function format_response($response, $context, $responseformat, $answerid, $options)
    {
        ...
        $convertstudentresponse = file_rewrite_pluginfile_urls(
            $response,
            'pluginfile.php',
            $context->id,
            'mod_lesson',
    ...
    public function save_answers_files($context, $maxbytes, &$answer, $answereditor = '', $responseeditor = '')
    {
        ...
        if (isset($responseeditor['itemid'])) {
            $answer->response = file_save_draft_area_files(
                $responseeditor['itemid'],
                $context->id,
                'mod_lesson',
                'page_responses',
                $answer->id,
    ...
    public static function rewrite_answers_urls($answer, $rewriteanswer = true)
    {
        ...
        $context = context_module::instance($PAGE->cm->id);
        if ($rewriteanswer) {
            $answer->answer = file_rewrite_pluginfile_urls(
                $answer->answer,
                'pluginfile.php',
                $context->id,
                'mod_lesson',
        ...
        $answer->response = file_rewrite_pluginfile_urls(
            $answer->response,
            'pluginfile.php',
            $context->id,
            'mod_lesson',
    ...
    public function get_contents()
    {
        ...
        if (!empty($this->properties->contents)) {
            ...
            $context = context_module::instance($PAGE->cm->id);
            $contents = file_rewrite_pluginfile_urls(
            ...
            return format_text(
                $contents,
                $this->properties->contentsformat,
                array(
                    'context' => $context,
                    'noclean' => true,
                    'overflowdiv' => true
...
class lesson_page_answer extends lesson_base
{
    ...
    public function get_files($includedirs = true, $updatedsince = 0)
    {
        ...
        $responsefiles = $fs->get_area_files(
            $lesson->context->id,
            'mod_lesson',
            'page_responses',
            $this->properties->id,
