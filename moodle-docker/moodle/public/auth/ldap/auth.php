...
class auth_plugin_ldap extends auth_plugin_base
{
    ...
    function user_login($username, $password)
    {
        ...
        if (
            !$ldap_login && ($this->config->user_type == 'ad')
    ...
    function user_create($userobject, $plainpass)
    {
        ...
        switch ($this->config->user_type) {
            ...
            case 'ad':
                ...
                unset($newuser);
                $newuser['unicodePwd'] = mb_convert_encoding(
                    '"' . $extpassword . '"',
                    'UCS-2LE',
                    'UTF-8'
                );
    ...
    function loginpage_hook()
    {
        ...
        if (
            ($_SERVER['REQUEST_METHOD'] === 'GET'         // Only on initial GET of loginpage
            ...
            if (
                $this->config->ntlmsso_ie_fastpath == AUTH_NTLM_FASTPATH_YESATTEMPT ||
