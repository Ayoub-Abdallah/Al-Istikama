...
/* This file contains modifications by Ayoub Ben Chahla
...
class graph
{
  ...
  function draw_text()
  {
    ...
    if ($colour != 'none')
      $this->draw_rectangle($this->calculated['inner_border'], $colour, $this->parameter['inner_border_type']); // graph border
  }
  ...
  function init_x_ticks()
  {
    ...
    foreach ($this->calculated['x_axis']['text'] as $set => $value) {
      ...
      if ($this->parameter['x_axis_gridlines'] == 'auto')
        $this->calculated['x_plot'][$set] = (int) round($tickX);
  ...
  function bar($x, $y, $type, $size, $colour, $offset, $index, $yoffset)
  {
    ...
    if ($this->parameter['zero_axis'] != 'none') {
      ...
      if ($this->parameter['inner_border'] != 'none')
        $bottom -= 1; // 1 pixel above bottom if border is to be drawn.
      $u_left = (int) round($x_left + $offset);
      $u_right = (int) round($x_right + $offset - 1);
      $v = $this->calculated['boundary_box']['bottom'] - $y + $offset;
