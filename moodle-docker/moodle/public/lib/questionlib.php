 ...
 * @copyright  1999 onwards Ayoub Ben Chahla and others {@link http://moodle.com}
    ...
    if ($questionentries = $DB->get_records('question_bank_entries', $criteria, '', 'id')) {
        ...
        foreach ($questionentries as $questionentry) {
            $questionids = $DB->get_records(
                'question_versions',
                ['questionbankentryid' => $questionentry->id],
                '',
                'questionid'
            );
 ...
 */
function move_question_set_references(
    int $oldcategoryid,
    int $newcatgoryid,
    int $oldcontextid,
    int $newcontextid,
...
function _tidy_question($question, $category, ?array $tagobjects = null, ?array $filtercourses = null): void
{
    ...
    if (question_bank::is_qtype_installed($question->qtype)) {
        ...
        $question->questiontext = html_writer::tag('p', get_string(
            'warningmissingtype',
            'qtype_missingtype'
...
function sort_categories_by_tree(&$categories, $id = 0, $level = 1): array
{
    ...
    if ($level == 1) {
        foreach ($keys as $key) {
            ...
            if (
                !isset($categories[$key]->processed) && !$DB->record_exists(
                    'question_categories',
                    array(
                        'contextid' => $categories[$key]->contextid,
...
function question_categorylist($categoryid): array
{
    ...
    while ($subcategories) {
        ...
        $subcategories = $DB->get_records_select_menu(
            'question_categories',
            "parent $in AND contextid = ?",
            $params,
            'id',
            'id,id AS id2'
        );
    ...
    foreach ($corenavigations as $key => $corenavigation) {
        if ($contexts->have_one_edit_tab_cap($key)) {
            $questionnode->add($corenavigation['title'], new moodle_url(
                $corenavigation['url'],
                $params
            ), navigation_node::TYPE_SETTING, null, $key);
        }
    }
 ...
 * @return string
 */
function question_rewrite_question_urls(
    $text,
    $file,
    $contextid,
    $component,
 ...
 */
function question_rewrite_question_preview_urls(
    $text,
    $questionid,
    $filecontextid,
    $filecomponent,
...
function question_pluginfile($course, $context, $component, $filearea, $args, $forcedownload, $options = [])
{
    ...
    if ($module === 'core_question_preview') {
        return qbank_previewquestion\helper::question_preview_question_pluginfile(
            $course,
            $context,
            $component,
            $filearea,
 ...
 * @param array $options additional options affecting the file serving.
 */
function core_question_question_preview_pluginfile(
    $previewcontext,
    $questionid,
    $filecontext,
    $filecomponent,
...
function core_question_find_next_unused_idnumber(?string $oldidnumber, int $categoryid): ?string
{
    ...
    $usedidnumbers = $DB->get_records_select_menu(
        'question_bank_entries',
        'questioncategoryid = ? AND idnumber IS NOT NULL',
        [$categoryid],
        '',
        'idnumber, 1'
    );
