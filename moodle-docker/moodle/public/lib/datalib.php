...
function get_all_instances_in_courses($modulename, $courses, $userid = NULL, $includeinvisible = false)
{
    ...
    if (
        !$rawmods = $DB->get_records_sql("SELECT cm.id AS coursemodule, m.*, cw.section, cm.visible AS visible,
...
function instance_is_visible($moduletype, $module)
{
    ...
    if (!empty($module->id)) {
        ...
        if (
            $records = $DB->get_records_sql("SELECT cm.instance, cm.visible, cm.groupingid, cm.id, cm.course
 ...
 */
function update_field_with_unique_index(
    $table,
    $field,
    array $newvalues,
