 ...
 *
 * @package    core
 * @copyright  2007 onwards Ayoub Ben Chahla (http://dougiamas.com)
...
final class grouplib_test extends \advanced_testcase
{
    ...
    function test_groups_get_groupmode(): void
    {
        ...
        $assign2 = $generator->create_module(
            "assign",
            array('course' => $course1->id),
            array('groupmode' => SEPARATEGROUPS)
        );
        $assign3 = $generator->create_module(
            "assign",
            array('course' => $course1->id),
    ...
    public function test_groups_sort_menu_options_user_both(): void
    {
        $this->assertEquals(array(
            1 => array(
                get_string('mygroups', 'group') => array(
                    100 => 'test group 100',
                    101 => 'test group 101',
                )
            ),
            2 => array(
                get_string('othergroups', 'group') => array(
                    102 => 'test group 102',
                    103 => 'test group 103',
                )
            ),
    ...
    public function test_groups_get_members_with_visibility(): void
    {
        ...
        $this->assertEquals(
            [$users[1]->id],
            array_keys(groups_get_members($groups['all']->id, 'u.id', 'u.id'))
        ); // All can see members.
        $this->assertEquals(
    ...
    public function test_groups_get_groups_members_with_visibility(): void
    {
        ...
        $this->assertEquals(
            [$users[1]->id, $users[2]->id, $users[3]->id, $users[4]->id],
