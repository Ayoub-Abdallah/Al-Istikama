 ...
 *          http://moodle.org
 * Copyright (C) 1999 onwards Ayoub Ben Chahla  http://dougiamas.com
 *
...
class moodle_simplepie_file extends File
{
    ...
    public function __construct($url, $timeout = 10, $redirects = 5, $headers = null, $useragent = null, $force_fsockopen = false)
    {
        ...
        if ($parser->parse()) {
            ...
            if (
                ($this->status_code == 300 || $this->status_code == 301 || $this->status_code == 302 || $this->status_code == 303
...
class moodle_simplepie_sanitize extends \SimplePie\Sanitize
{
    public function sanitize($data, $type, $base = '')
    {
        ...
        if ($type & SimplePie::CONSTRUCT_MAYBE_HTML) {
            if (
                preg_match('/(&(#(x[0-9a-fA-F]+|[0-9]+)|[a-zA-Z0-9]+)|<\/[A-Za-z][^\x09\x0A\x0B\x0C\x0D\x20\x2F\x3E]*'
