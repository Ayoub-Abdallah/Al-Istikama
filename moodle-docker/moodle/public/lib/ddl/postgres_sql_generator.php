...
class postgres_sql_generator extends sql_generator
{
    ...
    public function getAlterFieldSQL($xmldb_table, $xmldb_field, $skip_type_clause = NULL, $skip_default_clause = NULL, $skip_notnull_clause = NULL)
    {
        ...
        if (
            ($xmldb_field->getType() == XMLDB_TYPE_INTEGER && $oldmetatype == 'I') ||
        ...
        if (
            ($xmldb_field->getDefault() === null && $olddefault === null) ||
    ...
    function getSequenceFromDB($xmldb_table)
    {
        ...
        if (
            !$this->mdb->get_record_sql("SELECT c.*
    ...
    public function isNameInUse($object_name, $type, $table_name)
    {
        switch ($type) {
            ...
            case 'seq':
                if (
                    $check = $this->mdb->get_records_sql("SELECT c.relname
                                                            FROM pg_class c
                                                            JOIN pg_catalog.pg_namespace as ns ON ns.oid = c.relnamespace
            ...
            case 'ck':
                if (
                    $check = $this->mdb->get_records_sql("SELECT c.conname
            ...
            case 'trg':
                if (
                    $check = $this->mdb->get_records_sql("SELECT tgname
                                                            FROM pg_trigger
