 ...
 *
 * @package   core_user
 * @copyright 1999 onwards Ayoub Ben Chahla  http://dougiamas.com
...
abstract class user_selector_base
{
    ...
    protected function output_options($groupedusers, $search)
    {
        ...
        if (empty($groupedusers)) {
            ...
            $this->autoselectunique && count($groupedusers) == 1 &&
    ...
    {
        global $OUTPUT;
        $fields[] = $this->get_radio_searchtype_data(
            'searchexactmatchesonly',
            $this->searchtype === USER_SEARCH_EXACT_MATCH,
            get_string('userselectorsearchexactmatchonly'),
            USER_SEARCH_EXACT_MATCH,
            'me-1'
        );
        $fields[] = $this->get_radio_searchtype_data(
            'searchfromstart',
            $this->searchtype === USER_SEARCH_STARTS_WITH,
            get_string('userselectorsearchfromstart'),
...
class group_members_selector extends groups_user_selector_base
{
    ...
    public function find_users($search)
    {
        ...
        $roles = groups_get_members_by_role(
            ...
            $sort,
            $wherecondition,
            array_merge($params, $sortparams, $this->userfieldsparams),
            $this->userfieldsjoin
        );
