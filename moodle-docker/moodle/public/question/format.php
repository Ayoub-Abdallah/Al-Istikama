...
class qformat_default
{
    ...
    protected function count_questions($questions)
    {
        ...
        foreach ($questions as $question) {
            if (
                !is_object($question) || !isset($question->qtype) ||
        ...
        foreach ($catnames as $key => $catname) {
            if ($parent == 0) {
                ...
                if ($key == (count($catnames) - 1) && $lastcategoryinfo) {
                    ...
                    if (
                        isset($lastcategoryinfo->info) && $lastcategoryinfo->info !== ''
                        && $category->info === ''
                    ) {
                    ...
                    // Same for idnumber.
                    if (
                        isset($lastcategoryinfo->idnumber) && $lastcategoryinfo->idnumber !== ''
                        && $category->idnumber === ''
                    ) {
    ...
    {
        return s(question_utils::to_plain_text(
            $question->questiontext,
