...
class qtype_multianswer extends question_type
{
    ...
    public function get_question_options($question)
    {
        ...
        $wrappedquestions = $DB->get_records_list(
            'question',
            'id',
            explode(',', $sequence),
            'id ASC'
    ...
    protected function initialise_question_instance(question_definition $question, $questiondata)
    {
        ...
        foreach ($questiondata->options->questions as $key => $subqdata) {
            ...
            if ($subqdata->qtype == 'multichoice') {
                $answerregs = array();
                if (
                    $subqdata->options->shuffleanswers == 1 && isset($questiondata->options->shuffleanswers)
    ...
    public function get_random_guess_score($questiondata)
    {
        ...
        foreach ($questiondata->options->questions as $key => $subqdata) {
            ...
            $fractionsum += question_bank::get_qtype(
                $subqdata->qtype
            )->get_random_guess_score($subqdata);
        }
...
function qtype_multianswer_extract_question($text)
{
    ...
    for (
        ...
        while (preg_match('/~?' . ANSWER_ALTERNATIVE_REGEX . '/s', $remainingalts, $altregs)) {
            ...
            if (
                !empty($answerregs[ANSWER_REGEX_ANSWER_TYPE_NUMERICAL])
                && preg_match(
                    '~' . NUMERICAL_ALTERNATIVE_REGEX . '~s',
        ...
        $question->questiontext['text'] = implode(
            "{#$positionkey}",
            ...
            foreach ($subquestions as $subquestion) {
                ...
                foreach ($subquestion->answer as $key => $answer) {
                    ...
                    if ($trimmedanswer !== '') {
                        ...
                        if (
                            $subquestion->qtype == 'multichoice' && $subquestion->single == 0 &&
