...
class qtype_calculated extends question_type
{
    ...
    public function import_datasets($question)
    {
        ...
        foreach ($question->dataset as $dataset) {
            ...
            if ($dataset->status == 'private') {
                ...
                if (
                    $sharedatasetdefs = $DB->get_records_select(
                        'question_dataset_definitions',
                        "type = '1'
                    AND " . $DB->sql_equal('name', '?') . "
                    AND category = ?
                    ORDER BY id DESC ",
                        [$dataset->name, $question->category]
                    )
    ...
    protected function initialise_calculated_answers(question_definition $question, stdClass $questiondata)
    {
        ...
        foreach ($questiondata->options->answers as $a) {
            $question->answers[$a->id] = new \qtype_calculated\qtype_calculated_answer(
                $a->id,
                $a->answer,
                $a->fraction,
                $a->feedback,
    ...
    public function display_question_editing_page($mform, $question, $wizardnow)
    {
        ...
        switch ($wizardnow) {
            ...
            case 'datasetitems':
                echo $OUTPUT->heading_with_help(
                    get_string('editdatasets', 'qtype_calculated'),
                    'questiondatasets',
                    'qtype_calculated'
                );
                break;
    ...
    public function custom_generator_tools_part($mform, $idx, $j)
    {
        ...
        $mform->addGroup(
            $minmaxgrp,
            'minmaxgrp',
            get_string('minmax', 'qtype_calculated'),
            ' - ',
            false
        );
        ...
        $mform->addElement(
            'select',
            "calclength[{$idx}]",
            get_string('calclength', 'qtype_calculated'),
            $precisionoptions
        );

        $distriboptions = [
            'uniform' => get_string('uniform', 'qtype_calculated'),
            'loguniform' => get_string('loguniform', 'qtype_calculated')
        ];
        $mform->addElement(
            'select',
            "calcdistribution[{$idx}]",
            get_string('calcdistribution', 'qtype_calculated'),
            $distriboptions
        );
    }
    ...
    public function custom_generator_tools($datasetdef)
    {
        ...
        if (
            ...
            $options = [
                'uniform' => get_string('uniformbit', 'qtype_calculated'),
    ...
    public function update_dataset_options($datasetdefs, $form)
    {
        ...
        if (
            empty($form->definition) || empty($form->calcmin)
    ...
    public function save_dataset_items($question, $fromform)
    {
        ...
        if (
            isset($addeditem->itemnumber) && $maxnumber < $addeditem->itemnumber
        ...
        $numbertoadd = 0;
        if (
            isset($fromform->addbutton) && $fromform->selectadd > 0 &&
            $maxnumber < self::MAX_DATASET_ITEMS
        ) {
    ...
    public function comment_on_datasetitems(
        ...
        foreach ($answers as $key => $answer) {
            ...
            if ($answer->min === '') {
                ...
                $correcttrue->true = '';
                if (
                    (float) $formattedanswer->answer < $answer->min ||
    ...
    public function dataset_options(
        $form,
        $name,
        $mandatory = true,
    ...
    public function construct_dataset_menus(
        $form,
        $mandatorydatasets,
        $optionaldatasets
    ) {
    ...
    public function save_dataset_definitions($form)
    {
        ...
        foreach ($defids as $defid) {
            ...
            if (0 != $datasetdef->category) {
                ...
                if (
                    $olderdatasetdefs = $DB->get_records_select(
                        'question_dataset_definitions',
        ...
        foreach ($defids as $defid) {
            ...
            if (isset($datasetdef->id)) {
                ...
                if (0 == $datasetdef->category) {
                    ...
                    $datasetdef->itemcount = 0;
                    $datasetdef->id = $DB->insert_record(
                        'question_dataset_definitions',
                        $datasetdef
                    );
                    // Copy the dataitems.
...
function qtype_calculated_calculate_answer(
    $formula,
    $individualdata,
    $tolerance,
    $tolerancetype,
