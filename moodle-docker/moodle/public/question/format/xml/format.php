 ...
 *
 * @package    qformat_xml
 * @copyright  1999 onwards Ayoub Ben Chahla {@link http://moodle.com}
...
class qformat_xml extends qformat_default
{
    ...
    public function import_headers($question)
    {
        ...
        // Question name.
        $qo->name = $this->clean_question_name($this->getpath(
            $question,
            array('#', 'name', 0, '#', 'text', 0, '#'),
            '',
            true,
        ...
        $generalfeedback = $this->import_text_with_files(
            $question,
            array('#', 'generalfeedback', 0),
            '',
        ...
        $qo->defaultmark = $this->getpath(
            $question,
            array('#', 'defaultgrade', 0, '#'),
            $qo->defaultmark
        );
        $qo->penalty = $this->getpath(
            $question,
            array('#', 'penalty', 0, '#'),
            $qo->penalty
        );
    ...
    public function import_combined_feedback($qo, $questionxml, $withshownumpartscorrect = false)
    {
        ...
        foreach ($fields as $field) {
            $qo->$field = $this->import_text_with_files(
                $questionxml,
                array('#', $field, 0),
                '',
                $this->get_format($qo->questiontextformat)
        ...
        if ($withshownumpartscorrect) {
            ...
            if (array_key_exists('correctresponsesfeedback', $questionxml['#'])) {
                $qo->shownumcorrect = $this->trans_single($this->getpath(
                    $questionxml,
                    array('#', 'correctresponsesfeedback', 0, '#'),
                    1
                ));
            }
        }
    ...
    public function import_hint($hintxml, $defaultformat)
    {
        ...
        if (array_key_exists('hintcontent', $hintxml['#'])) {
            ...
            $hint->hint = $this->import_text_with_files(
                $hintxml,
                array('#', 'hintcontent', 0),
                '',
                $defaultformat
            ...
            $hint->shownumcorrect = $this->getpath(
                $hintxml,
                array('#', 'statenumberofcorrectresponses', 0, '#'),
                0
            );
            $hint->clearwrong = $this->getpath(
                $hintxml,
                array('#', 'clearincorrectresponses', 0, '#'),
                0
            );
            $hint->options = $this->getpath(
                $hintxml,
                array('#', 'showfeedbacktoresponses', 0, '#'),
                0
            );
    ...
    public function import_multichoice($question)
    {
        ...
        $shuffleanswers = $this->getpath(
            $question,
            array('#', 'shuffleanswers', 0, '#'),
            'false'
        );
        $qo->answernumbering = $this->getpath(
            $question,
            array('#', 'answernumbering', 0, '#'),
            'abc'
        );
        $qo->shuffleanswers = $this->trans_single($shuffleanswers);
        $qo->showstandardinstruction = $this->getpath(
            $question,
    ...
    public function import_multianswer($question)
    {
        ...
        if (isset($question['#']['name'])) {
            ...
            $qo->name = $this->create_default_question_name(
                $qo->questiontext['text'],
    ...
    public function import_truefalse($question)
    {
        ...
        foreach ($question['#']['answer'] as $answer) {
            $answertext = $this->getpath(
                $answer,
                array('#', 'text', 0, '#'),
                '',
                true
            );
            $feedback = $this->import_text_with_files(
                $answer,
                array('#', 'feedback', 0),
                '',
                $this->get_format($qo->questiontextformat)
    ...
    public function import_numerical($question)
    {
        ...
        if (!empty($instructions)) {
            $qo->instructions = $this->import_text_with_files(
                $instructions,
                array('0'),
                '',
                $this->get_format($qo->questiontextformat)
    ...
    public function import_match($question)
    {
        ...
        $qo->qtype = 'match';
        $qo->shuffleanswers = $this->trans_single($this->getpath(
            $question,
            array('#', 'shuffleanswers', 0, '#'),
            1
        ));
        ...
        foreach ($question['#']['subquestion'] as $subqxml) {
            $qo->subquestions[] = $this->import_text_with_files(
                $subqxml,
                array(),
                '',
                $this->get_format($qo->questiontextformat)
            ...
            $qo->subanswers[] = $this->getpath(
                $subqxml,
                array('#', 'answer', 0, '#', 'text', 0, '#'),
                '',
                true
            );
        }
    ...
    public function import_essay($question)
    {
        ...
        $qo->responseformat = $this->getpath(
            $question,
            array('#', 'responseformat', 0, '#'),
            'editor'
        );
        $qo->responsefieldlines = $this->getpath(
            $question,
            array('#', 'responsefieldlines', 0, '#'),
            15
        );
        $qo->responserequired = $this->getpath(
            $question,
            array('#', 'responserequired', 0, '#'),
            1
        );
        $qo->minwordlimit = $this->getpath(
            $question,
            array('#', 'minwordlimit', 0, '#'),
            null
        );
        $qo->minwordenabled = !empty($qo->minwordlimit);
        $qo->maxwordlimit = $this->getpath(
            $question,
            array('#', 'maxwordlimit', 0, '#'),
            null
        );
        $qo->maxwordenabled = !empty($qo->maxwordlimit);
        $qo->attachments = $this->getpath(
            $question,
            array('#', 'attachments', 0, '#'),
            0
        );
        $qo->attachmentsrequired = $this->getpath(
            $question,
            array('#', 'attachmentsrequired', 0, '#'),
            0
        );
        $qo->filetypeslist = $this->getpath(
            $question,
            array('#', 'filetypeslist', 0, '#'),
            null
        );
        $qo->maxbytes = $this->getpath(
            $question,
            array('#', 'maxbytes', 0, '#'),
            null
        );
        $qo->graderinfo = $this->import_text_with_files(
            $question,
            array('#', 'graderinfo', 0),
            '',
            $this->get_format($qo->questiontextformat)
        );
        $qo->responsetemplate['text'] = $this->getpath(
            $question,
            array('#', 'responsetemplate', 0, '#', 'text', 0, '#'),
            '',
            true
        );
        $qo->responsetemplate['format'] = $this->trans_format($this->getpath(
            $question,
            array('#', 'responsetemplate', 0, '@', 'format'),
            $this->get_format($qo->questiontextformat)
        ));
    ...
    public function import_calculated($question)
    {
        ...
        foreach ($datasets as $dataset) {
            ...
            $qo->dataset[$qo->datasetindex]->number_of_items = $this->getpath(
                $dataset,
                array('#', 'number_of_items', 0, '#'),
                0
            );
            $datasetitems = $this->getpath(
                $dataset,
                array('#', 'dataset_items', 0, '#', 'dataset_item'),
                array()
            );
    ...
    public function writequestion($question)
    {
        ...
        switch ($question->qtype) {
            ...
            case 'essay':
                ...
                $expout .= $this->write_files($fs->get_area_files(
                    $contextid,
                    'qtype_essay',
                    'graderinfo',
                    $question->id
