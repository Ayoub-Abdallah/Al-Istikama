...
/**
 * Strings for component 'admin', language 'ar', version '5.1'.
 *
 * @package     admin
 * @category    string
...
$string['composerdeveloperdependenciesinstalled'] = 'تم تنصيب تبعات مطور Composer. تأكد من أن الأمر "composer install --no-dev --classmap-authoritative" قد تم تشغيله في مجلد جذر مودل.';
$string['composernotfound'] = 'تعذر العثور على تبعات Composer. يرجى التأكد من أن الأمر "composer install --no-dev --classmap-authoritative" قد تم تشغيله في مجلد جذر مودل. إذا لم تكن تستعمل Composer، تأكد من وجود مجلد البائع وأنه يحتوي على الملفات الضرورية.';
$string['composernotoptimised'] = 'إن مودل يعمل في وضعية الإنتاج في حين إن معالج التحميل التلقائي لـ Composer لم يتم تحسينه بعد. قد ترغب في تنفيذ الأمر "composer install --no-dev --classmap-authoritative" لتحسين أداء التحميل التلقائي.';
$string['composeroptimisedindevmode'] = 'إن التحميل التلقائي لـ Composer يجري تشغيله حاليًا في حين إن مودل يعمل في وضعية التطوير. هذا من شأنه التسبب في مشاكل أحيانًا. قد ترغب في تنفيذ الأمر "composer install" بدون أي عوامل إضافية.';
...
$string['configyuicomboloading'] = 'هذه الخيارات تُمَكن الجمع بين تحميل الملفات الأمثل لمكتبات YUI. وينبغي تمكين هذا الإعداد لأسباب تتعلق بالأداء.';
...
$string['courserequestnotifyemail'] = '<p>المستخدم {$a->user} قام بطلب مساق جديد.</p>
<p>يمكنك مراجعة الطلب من خلال <a href="{$a->link}">المساقات التي تنتظر الموافقة</a>.</p>

<p>تفاصيل الطلب:</p>
<ul>
<li>الاسم الكامل للمساق: {$a->fullname}</li>
<li>الاسم المختصر للمساق: {$a->shortname}</li>
<li>التصنيف: {$a->category}</li>
<li>سبب طلب هذا المساق: {$a->reason}</li>
</ul>';
